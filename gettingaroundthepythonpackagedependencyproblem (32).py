...
def prep_data():
    ...
    return tokenized_s1, tokenized_s2, tokenized_gold, vtokenizer, filtered_data
...
def step_3(tokenized_s1, tokenized_s2, tokenized_gold, vtokenizer):
    ...
    return cls_embed_ts1p, cls_embed_ts1p_no_preps, sentnum_s1, preps_ids_to_remove
...
def vector_summarization_step(cls_embed_ts1p, cls_embed_ts1p_no_preps, sentnum_s1, cls_embed_ts2p, cls_embed_ts2p_no_preps, sentnum_s2):
    ...
    #this is all just wrong so ignore it
...
def matchmaker(sentnum_s1, sentnum_s2):


  print ("match maker")

  #print(type(sentnum_s1[0])) bro it is an integer


  #remmeber that there are less elements in sentnum2 than there are sentnum1 so doin't go over with iteration variable, stop


  #this loop should look/get any matches between sentnum_s1 and sentnum_s2

  matches_s1_s2 = []

  print(len(sentnum_s1)) #177   #of by one makes it 0 to 176
  print(len(sentnum_s2))  #90    of by one makes it 0 to 89

  #this makes a copy of sentnum_s2 so I can add some padding values so the fucking loop won't keep over-indexing
  sentnum_s2_adjusted = sentnum_s2.copy()

  print(len(sentnum_s2_adjusted))  #90

  #loop to add padding to sentnum_s2_adjusted so that it is same size as sentnum_s1
  #padding value will be -1
  #I need to add 87 pads



  #length_to_pad = len(sentnum_s1) - len(sentnum_s2) #gets diff for pad

  for i in range(len(sentnum_s1) - len(sentnum_s2)):
    sentnum_s2_adjusted.append(-1)





  print(len(sentnum_s2_adjusted))
  print(sentnum_s2_adjusted)




  for i in range(len(sentnum_s1)):
    for j in range(len(sentnum_s2_adjusted)):
      if sentnum_s1[i] == sentnum_s2_adjusted[j]:
        matches_s1_s2.append(sentnum_s1[i])
  ...
  return matches_s1_s2, sentnum_s2_adjusted





def puller(the_sentnum, tokenized_og):

  print("fdddddd")
  #bruh imma be hoping this OOP works ----> OOP = POOP

  matched_set_to_return = []
  num_tracker = []


  for i in range(len(the_sentnum)):
    print(tokenized_og[the_sentnum[i]])
    matched_set_to_return.append(tokenized_og[the_sentnum[i]])


  print(matched_set_to_return)

  print(len(matched_set_to_return))
  print(len(the_sentnum))

  if len(matched_set_to_return) == len(the_sentnum):
    print("yes yes")
  else:
    print("OWO NO NO NO NO NO OWO")

  return matched_set_to_return



def get_cls_embeds_for_matched(matched_token_input, vtokenizer):
  print("ddd")


  #convert matched_token_inputs to nparray

  #print(matched_token_input)
  print(type(matched_token_input)) #is a list of tensors
  print(type(matched_token_input[0]))  #is a tensor
  ...
  for tensor in matched_token_input:
    token_ids = tensor.numpy().flatten().tolist()
    gleengleen = [tokid for tokid in token_ids]
    matched_token_ppp.append(gleengleen)


  #print(matched_token_ppp) #these don't get utlitized
  print(type(matched_token_ppp)) #is a list of lists
  print(type(matched_token_ppp[0])) #is a list


  #print(vtokenizer.convert_tokens_to_string(vtokenizer.convert_ids_to_tokens(matched_token_input[0])))

  '''

  for i, sentence in enumerate(matched_token_input[0]):
    print(vtokenizer.convert_tokens_to_string(vtokenizer.convert_ids_to_tokens(sentence)))

  for k in range(len(matched_token_input)):
    for i, sentence in enumerate(matched_token_input[k]):
      print(vtokenizer.convert_tokens_to_string(vtokenizer.convert_ids_to_tokens(sentence)))
  ...
  #padds it  and gives last hid state and last hid state shape

  matched_token_list = [tensor.numpy().flatten().tolist() for tensor in matched_token_input]

  matched_token_input_padded = tf.keras.preprocessing.sequence.pad_sequences(matched_token_list, padding = "post")



  #in its current state, this line gives an error saying "Shape of sample (18,) of sequence at position 2 is different from expected shape (15,)""
  #matched_token_input_padded = tf.keras.preprocessing.sequence.pad_sequences(matched_token_input, padding = "post" )


  #matched_token_input_padded should already be in tensor format

  #matched_token_input_padded = tf.convert_to_tensor(matched_token_input_padded)
  #matched_token_input = tf.convert_to_tensor(matched_token_input) #setting it back just in case


  #logically consistent with previous code but syntatically doesn't work
  #matched_token_input_padded_tensor = tf.convert_to_tensor(sentence, dtype=tf.int32)

  print("shape for matched_token_input_padded:")
  print(matched_token_input_padded.shape)




  #syntatically correct but not sure if logically correct
  matched_token_input_padded_tensor = tf.convert_to_tensor(matched_token_input_padded)



  #July 22nd Update (uncomment this when you fix batch_sequence error)

  matched_outputs = bert_model(matched_token_input_padded_tensor)

  last_hid_state_matched = matched_outputs.last_hidden_state

  print(f"shape of last hidden state of {matched_outputs}")

  print(last_hid_state_matched.shape)


  last_hid_state_matched_cls = last_hid_state_matched[:,0,:]

  print(f"shape of cls last hidden state of {matched_outputs}")

  print(last_hid_state_matched_cls.shape)

  #returns the cls embeds of the matched_outputs so I can shove them into a file in another function




  return last_hid_state_matched_cls, matched_outputs
  ...
  #prints out text for seeing it
  for i in matched_token_ppp:
    print(vtokenizer.convert_tokens_to_string(vtokenizer.convert_ids_to_tokens(matched_token_ppp[i])))


  tensors_of_matched_token_input = [tf.convert_to_tensor(sentence, dtype=tf.int32) for sentence in matched_token_ppp]
  '''
...
def file_maker(cls_embeds_of_2_matched_to_1, cls_embeds_of_1_matched_to_2, sentnum_s1, sentnum_s2):


  print("finally")

  #remember to use sentnum_s1 on 2 to 1 and sentnum_s2 on 1 to 2

  with open ("cls_embeds_of_2_matched_to_1.txt", "w", encoding = "utf-8") as file:
    for index, embedding in enumerate(cls_embeds_of_2_matched_to_1):
      file.write(f"This is the CLS embedding for sentence 2 that matches to sentence 1 number {sentnum_s1[index]} in the original JSON file: \n")
      file.write(f"CLS embedding: {embedding.numpy().tolist()} \n\n")



  with open ("cls_embeds_of_1_matched_to_2.txt", "w", encoding = "utf-8") as file:
    for index, embedding in enumerate(cls_embeds_of_1_matched_to_2):
      file.write(f"This is the CLS embedding for sentence 1 that matches to sentence 2 number {sentnum_s2[index]} in the original JSON file: \n")
      file.write(f"CLS embedding: {embedding.numpy().tolist()} \n\n")



def file_maker_for_no_preps(cls_embeds_of_2_matched_to_1_nopreps, cls_embeds_of_1_matched_to_2_nopreps, sentnum_s1, sentnum_s2):
  print("uwu")

  with open ("cls_embeds_of_2_matched_to_1_nopreps.txt", "w", encoding = "utf-8") as file:
    for index, embedding in enumerate(cls_embeds_of_2_matched_to_1_nopreps):
      file.write(f"This is the CLS embedding for sentence 2 that matches to sentence 1 number without selected prepositions{sentnum_s1[index]} in the original JSON file: \n")
      file.write(f"CLS embedding: {embedding.numpy().tolist()} \n\n")



  with open ("cls_embeds_of_1_matched_to_2_nopreps.txt", "w", encoding = "utf-8") as file:
    for index, embedding in enumerate(cls_embeds_of_1_matched_to_2_nopreps):
      file.write(f"This is the CLS embedding for sentence 1 that matches to sentence 2 number without selected prepositions {sentnum_s2[index]} in the original JSON file: \n")
      file.write(f"CLS embedding: {embedding.numpy().tolist()} \n\n")




def remove_preps_from_pulled_matches(matched_tokens, matched_embedding, vtokenizer, preps_ids_to_remove):

  print("gotta get it done")

  #[tf.convert_to_tensor([part for part in sentence.numpy() if part not in preps_ids_to_remove], dtype=tf.int32) for sentence in ts2p]

  print("printing out dtype of the matched_embeddings that were input to the function: ")
  print(matched_embedding.last_hidden_state.dtype) #this has a dtype of float32


  print("matched_embedding type: ")
  print(type(matched_embedding)) #this is a class 'transformers.modeling_tf_outputs.TFBaseModelOutputWithPoolingAndCrossAttentions'>

  print(type(matched_embedding[0])) #this is a <class 'tensorflow.python.framework.ops.EagerTensor'>

  #make sure that sentence is the correct type and not a string!


  print(type(matched_tokens)) #list
  print(type(matched_tokens[0])) #tensors
  #print(matched_tokens.shape)
  #print(matched_tokens[0].shape)




  #is that dtype = tf.int32 gonna be a problem cuz the matched_embedding was not set to that?

  matched_tokens_nopreps = [tf.convert_to_tensor([part for part in sentence.numpy().flatten().tolist() if part not in preps_ids_to_remove], dtype = tf.int32) for sentence in matched_tokens]

  #comparison time


  print(type(matched_tokens[0])) #tensor
  print(type(matched_tokens_nopreps[0]))  #tensor

  print(matched_tokens[0].shape)
  print(matched_tokens_nopreps[0].shape)



  if len(matched_tokens) == len(matched_tokens_nopreps):
    print("the lengths match/n")
    for i in range(len(matched_tokens)):
      ids_matched = matched_tokens[i].numpy().flatten().tolist()
      print(vtokenizer.convert_tokens_to_string(vtokenizer.convert_ids_to_tokens(ids_matched)))
      print("\n")
      ids_matched_nopreps = matched_tokens_nopreps[i].numpy().flatten().tolist()
      print(vtokenizer.convert_tokens_to_string(vtokenizer.convert_ids_to_tokens(ids_matched_nopreps)))
      print("\n\n\n")



  #print(vtokenizer.convert_tokens_to_string(vtokenizer.convert_ids_to_tokens(matched_tokens[0])))
  #print(vtokenizer.convert_tokens_to_string(vtokenizer.convert_ids_to_tokens(matched_tokens_nopreps[0])))


  return matched_tokens_nopreps



def dtype_checker(input_cls_embedding):

  print("dtype of input_cls_embedding: ")
  print(input_cls_embedding.dtype)
  print("\n")


def type_checker(input_cls_embedding):

  print("input_cls_embedding type: ")
  print(type(input_cls_embedding))
  print("\n")

def shape_checker(input_cls_embedding):

  print("input_cls_embedding shape: ")
  print(input_cls_embedding.shape)
  print("\n")


def exportation(input_cls_embedding, file_name):

  print("final step yay!")

  #https://www.tensorflow.org/api_docs/python/tf/io/gfile/GFile

  #serialized = tf.io.serialize_tensor(input_cls_embedding)

  with tf.io.gfile.GFile(file_name + ".bin", "wb") as f:
    for tensor in input_cls_embedding:
      serialized = tf.io.serialize_tensor(tensor)
      serialized_bytes = serialized.numpy() #also stole this line from ChatGPT
      f.write(len(serialized_bytes).to_bytes(4, 'big')) #I stole this line from ChatGPT
      f.write(serialized_bytes)


#Whoops, I am back because I forgot to take some stuff with me (July 27th, 2025, at 5:00PM)
def export_sentnum(sentnum_input, desired_file_name):
  print("to export sentnum in np.array because I need it")
  #remember that sentnum_s1 goes to P and CS2
  #remember that sentnum_s2 goes to CS1 and H

  np.save(desired_file_name + ".npy", sentnum_input )



#takes gold_labels from original data set because I need them! Stores them in np.array
def get_gold_labels(filtered_data):
  print("this takes gold_labels")

  gold_labels_to_return = []

  for entry in filtered_data:
        sentence_from_data = entry["gold_label"]
        gold_labels_to_return.append(sentence_from_data)


  gold_labels_as_np_array = np.array(gold_labels_to_return)

  np.save("gold_labels.npy", gold_labels_as_np_array)
...
def main():
    tokenized_s1, tokenized_s2, tokenized_gold, vtokenizer, filtered_data = prep_data()
    #July 22 note, added preps_ids_to_remove as an additional return to the step_3 function, as I now need that data elsewhere
    cls_embed_ts1p, cls_embed_ts1p_no_preps, sentnum_s1, preps_ids_to_remove = step_3(tokenized_s1, tokenized_s2, tokenized_gold, vtokenizer)
    ...
    #find any matches between SentNum 1 and SentNum 2

    matches_s1_s2, sentnum_s2_adjusted = matchmaker(sentnum_s1, sentnum_s2)
    ...
    #use SentNum1 to pull from 2
    #Use SentNum2 to pull from 1

    #so imma gonna pull out the correspoding sentences for each pair,
    #i'll have two sets of corresponding sentences, one pulled from sentence 1 data that pairs  to my twin sentence 2's data sets (one twin with preps, and the other without)
    #and the other of pulled sentence 2 data that pairs with my twin sentence 1 sets (prep and no prep)
    #example:

    '''


        pulled sentence              pre-existing data that matches to it, (to get cls collapsed)

         s1_pulled ----->             s2_preps  &  s2_no_preps
         s2_pulled ----->             s1_preps  &  s1_no_preps




    then i'll have four files with combined cls tokens
    ...
    #this gets the tokens for the CS2 by using the sentnum1 to pull corresponding sentences from S2
    tok2_matched_to_s1 = puller(sentnum_s1, tokenized_s2)
    #this gets the tokens for the CS1 by using the sentnum2 to pull corresponding sentences from S1
    tok1_matched_to_s2 = puller(sentnum_s2, tokenized_s1)

    print(sentnum_s1)
    print(sentnum_s2)

    print(tok2_matched_to_s1)
    print(tok1_matched_to_s2)


    print(tf.__version__)

    #July 2nd update (UNCOMMENT THESE FUNCTIONS WHEN YOU FIX THE ERROR)
    #July 22nd update (we fixed the error!)

    #this gets the cls embeds of the CS2 that are supposed to match to P and P'
    cls_embeds_of_2_matched_to_1, matched_2_1_embeddings = get_cls_embeds_for_matched(tok2_matched_to_s1, vtokenizer)

    #this gets the cls embeds of the CS1 that are supposed to match to H and H'
    cls_embeds_of_1_matched_to_2, matched_1_2_embeddings = get_cls_embeds_for_matched(tok1_matched_to_s2, vtokenizer)



    #I coulda stream lined some of this by making a dictionary, oh well

    file_maker(cls_embeds_of_2_matched_to_1, cls_embeds_of_1_matched_to_2, sentnum_s1, sentnum_s2)

    tok2_matched_to_s1_nopreps = remove_preps_from_pulled_matches(tok2_matched_to_s1,matched_2_1_embeddings, vtokenizer, preps_ids_to_remove)
    tok1_matched_to_s2_nopreps = remove_preps_from_pulled_matches(tok1_matched_to_s2, matched_1_2_embeddings, vtokenizer, preps_ids_to_remove)


    cls_embeds_of_2_matched_to_1_nopreps, matches_2_1_embeddings_nopreps = get_cls_embeds_for_matched(tok2_matched_to_s1_nopreps, vtokenizer)
    cls_embeds_of_1_matched_to_2_nopreps, matches_1_2_embeddings_nopreps = get_cls_embeds_for_matched(tok1_matched_to_s2_nopreps, vtokenizer)




    #testing out data types
    #July 24th, 1:13am update; something is very wrong
    #July 24th, 11:09am update; holy fudge sticks I hate tuples so much

    print(type(cls_embeds_of_2_matched_to_1)) #tensorflow tensor
    print(type(cls_embeds_of_1_matched_to_2)) #tensorflow tensor

    print(type(cls_embeds_of_2_matched_to_1_nopreps))  #tuple  <-- these should NOT be tuples!
    print(type(cls_embeds_of_1_matched_to_2_nopreps))  #tuple


    file_maker_for_no_preps(cls_embeds_of_2_matched_to_1_nopreps, cls_embeds_of_1_matched_to_2_nopreps, sentnum_s1, sentnum_s2)



    dtype_checker(cls_embed_ts1p)   #this is P
    dtype_checker(cls_embed_ts1p_no_preps)   #this is P'
    dtype_checker(cls_embed_ts2p)     #this is H
    dtype_checker(cls_embed_ts2p_no_preps)     #this is H'
    dtype_checker(cls_embeds_of_2_matched_to_1)    #this is CS2
    dtype_checker(cls_embeds_of_2_matched_to_1_nopreps)   #this is CS2'
    dtype_checker(cls_embeds_of_1_matched_to_2)    #this is CS1
    dtype_checker(cls_embeds_of_1_matched_to_2_nopreps)    #this is CS1'

    type_checker(cls_embed_ts1p)   #this is P
    type_checker(cls_embed_ts1p_no_preps)   #this is P'
    type_checker(cls_embed_ts2p)     #this is H
    type_checker(cls_embed_ts2p_no_preps)     #this is H'
    type_checker(cls_embeds_of_2_matched_to_1)    #this is CS2
    type_checker(cls_embeds_of_2_matched_to_1_nopreps)   #this is CS2'
    type_checker(cls_embeds_of_1_matched_to_2)    #this is CS1
    type_checker(cls_embeds_of_1_matched_to_2_nopreps)    #this is CS1'


    shape_checker(cls_embed_ts1p)   #this is P
    shape_checker(cls_embed_ts1p_no_preps)   #this is P'
    shape_checker(cls_embed_ts2p)     #this is H
    shape_checker(cls_embed_ts2p_no_preps)     #this is H'
    shape_checker(cls_embeds_of_2_matched_to_1)    #this is CS2
    shape_checker(cls_embeds_of_2_matched_to_1_nopreps)   #this is CS2'
    shape_checker(cls_embeds_of_1_matched_to_2)    #this is CS1
    shape_checker(cls_embeds_of_1_matched_to_2_nopreps)    #this is CS1'



    #this should export the embeds to their own files named as per the google doc


    exportation(cls_embed_ts1p, "P")
    exportation(cls_embed_ts1p_no_preps, "P_")   #this is P'
    exportation(cls_embed_ts2p, "H")     #this is H
    exportation(cls_embed_ts2p_no_preps, "H_")     #this is H'
    exportation(cls_embeds_of_2_matched_to_1, "CS2")    #this is CS2
    exportation(cls_embeds_of_2_matched_to_1_nopreps, "CS2_")   #this is CS2'
    exportation(cls_embeds_of_1_matched_to_2, "CS1")    #this is CS1
    exportation(cls_embeds_of_1_matched_to_2_nopreps, "CS1_")    #this is CS1'



    export_sentnum(sentnum_s1, "sentnum_s1")
    export_sentnum(sentnum_s2, "sentnum_s2")
    get_gold_labels(filtered_data)
    ...
    #Tokenized those pulls
    #get embeddings of those pulls
    #get just CLS embeddings of those pulls


    #if there is a matching, should the no_prep version have the preposiont removed? No! actually, cuz it would test understanding?
